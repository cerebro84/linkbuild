...
public class ConnectionServer 
{
    ...
    //Costanti che definiscono il tipo di disconnessione possibile per un client
    final static int PING = 100;
    final static int SYSBAN = 105;
    final static int APPBAN = 110;
    final static int NORMAL = 120;
    final static int ABNORMAL = 130;
    final static int REGISTRATION = 140;
        
    //Variables
    private static ConnectionServer instance = null; // singletone instance
    private SettingsManager sm; //SettingsManager instance
    private TimerPing ourPing;
    private ConnectionThread listeningThread; //to track the unconnected Thread
    ...
    {
        sm = SettingsManager.getInstance(); //We get an instance now to avoid calling SettingsManager.getInstance() every time
    ...
    protected boolean addConnection(String app,String address, ConnectionThread process)
    {
        ...
        {
            if (!BanManager.getInstance().couldAccess(address, app)) //se non può accedere allora mi fermo
            {
    ...
    {
        (listeningThread = new ConnectionThread()).start(); // Opens a new listening thread
    }
    ...
    protected boolean removeConnection (String address, int type)
    {
        try 
        {
            ...
            if (deleted)
                MainListener.getInstance().displayTrayMessage("User id "+ address  + " disconnected", false);
    ...
    public boolean send (int password, String [] data, String address) throws AccessControlException 
    {
        try 
        {
            ...
            if(sm.couldAllowToManageUser(password, address) && data != null)
                return sm.getAssociatedProcess(address).send(data);
        } 
    ...
    */
    public boolean send (int password, byte data, String address) throws AccessControlException
    {
        try 
        {
            if(sm.couldAllowToManageUser(password, address))
                return sm.getAssociatedProcess(address).send(data);
        } 
        catch (IOException ex) 
        {
    ...
    */
    ...
    protected boolean receive (String data, String address)
    {
        ...
        if (!BanManager.getInstance().manageBanningFor(address, sm.getApplicationUsedByUser(address), data))
            return false;
    ...
    protected static boolean onClosing()
    {
        ...
        else
        {
            ...
            if (instance.listeningThread != null) 
                instance.listeningThread.onClosing();
            for (ConnectionThread connection : instance.getRunningThreads())
